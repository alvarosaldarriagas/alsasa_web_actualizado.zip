...
import { getBridgePosts, getBridgeGallery, propertyCode, mapWordPressPost, mergeCatalog, mergePhotoGallery, mergeWordPressPrice } from "@/lib/wordpress-bridge.mjs";
...
export async function getProperties() {
    const postsPromise = getBridgePosts();
    try {
        const base = await getBase44Properties();
        return mergeCatalog(base, await postsPromise);
    } catch (error) {
        console.error('Base44 inventory fallback to WordPress:', error);
        const [legacy, posts] = await Promise.all([getWordPressProperties(), postsPromise]);
        // Keep the existing fallback, but give approved additions their canonical codes.
        const additions = posts.map(post => mapWordPressPost(post)).filter(Boolean);
        return mergeCatalog(legacy.filter(item => !additions.some(p => p.wordpressId === item.id)), posts);
    }
}

export async function getPropertyById(id) {
    const code = String(id).trim().toUpperCase();
    const isBridgeCode = ["A1149", "A1163", "A1164", "A1165", "A1166", "A1167"].includes(code);
    const postsPromise = isBridgeCode ? getBridgePosts() : Promise.resolve([]);
    let base;
    try {
        base = await getBase44PropertyById(id);
    } catch (error) {
        console.error('Base44 property fallback to WordPress:', error);
    }
    const post = (await postsPromise).find(item => propertyCode(item) === code);
    if (base) {
        return code === "A1149" && post ? mergePhotoGallery(mergeWordPressPrice(base, post), await getBridgeGallery(post)) : base;
    }
    if (post && code !== "A1149") return mapWordPressPost(post, await getBridgeGallery(post));
