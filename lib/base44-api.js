...
function mapProperty(property) {
  ...
  return {
    id: property.custom_id || property.id,
...
export async function getBase44PropertyById(id) {
  const parameter = /^[a-f0-9]{24}$/i.test(String(id)) ? "id" : "custom_id";
  const payload = await request(`?action=detail&${parameter}=${encodeURIComponent(id)}`);
