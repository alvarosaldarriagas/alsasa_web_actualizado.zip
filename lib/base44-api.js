...
export function normalizePropertyCode(value) {
  ...
  try {
    ...
    // Conserva el valor original si llega una codificación incompleta.
  }
  const legacyMatch = code.match(/^ID:\s*(A\d+)$/i);
  if (legacyMatch) return legacyMatch[1].toUpperCase();
  return /^A\d+$/i.test(code) ? code.toUpperCase() : code;
}

function mapProperty(property) {
  const gallery = normalizeImages(property.images);
  const customId = normalizePropertyCode(property.custom_id);
  ...
  return {
    id: customId || property.id,
    base44Id: property.id,
    customId,
    title: property.title || "Propiedad ALSASA",
    link: `/propiedad/${customId || property.id}`,
    image: gallery[0] || "",
    gallery,
    price,
...
export async function getBase44PropertyById(id) {
  ...
  const parameter = /^[a-f0-9]{24}$/i.test(normalizedId) ? "id" : "custom_id";
  const payload = await request(`?action=detail&${parameter}=${encodeURIComponent(normalizedId)}`);
