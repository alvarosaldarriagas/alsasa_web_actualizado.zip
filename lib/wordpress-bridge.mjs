...
export function approvedPosts(posts) {
  const candidates = posts.filter(post => post.status === "publish" &&
    (Object.hasOwn(ADDITIONS, propertyCode(post)) || propertyCode(post) === PHOTO_ONLY));
...
export function mapWordPressPost(post, gallery = []) {
  ...
  return {
    ...
    location: ADDITIONS[code], action: "Venta",
