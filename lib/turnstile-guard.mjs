...
export async function protectRequest(request, kind, handle, {
  env = process.env, fetcher = fetch, now = Date.now, admitIngress,
} = {}) {
  ...
  let context;
  try {
    const admit = admitIngress ?? (await import('./web-guard-runtime.mjs')).admitWebIngress;
    context = await admit(request, { kind, operation: body.operation, env });
  } catch {
    return guardError(503, 'La verificación está temporalmente pausada. Puedes contactarnos por WhatsApp.');
  }
  if (context?.allowed !== true) return guardError(429, 'Se alcanzó el límite de solicitudes. Intenta más tarde.');
  if (!/^[a-f0-9]{64}$/.test(context.subject)) return guardError(503, 'Verificación no disponible.');
  ...
  const response = await handle(payload, { operation, kind, subject: context.subject });
