...
// candidates/base44/entries/publicApi.ts
...
function createCaptureRouter({ kind, receiver: receiver2, readPublic: readPublic2 } = {}) {
  ...
  return async (request) => {
    ...
    if (["GET", "OPTIONS"].includes(request.method) && kind === "form" && typeof readPublic2 === "function") return readPublic2(request);
...
function createReceiverRuntime({ kind, env, Pool: Pool2, createAxiosClient: createAxiosClient2, createEntitiesModule: createEntitiesModule2, readPublic: readPublic2 } = {}) {
  let ring;
  const pools = [];
  const closed = createCaptureRouter({ kind, readPublic: readPublic2 });
  const close = async () => {
    ring?.destroy();
  ...
  try {
    ...
    return { handle: createCaptureRouter({ kind, receiver: signed, readPublic: readPublic2 }), close };
  } catch {
    void close();
...
var { Pool } = pg;
var PublicInputError = class extends Error {
  status;
...
function publicText(value, max, optional = true) {
  ...
  if (typeof value !== "string" || value.length > max) {
    throw new PublicInputError("Texto inv\xE1lido o demasiado largo.");
  }
  const text2 = value.trim();
  if (!optional && !text2) throw new PublicInputError("Falta un campo obligatorio.");
  return text2;
}
function optionalBudget(value) {
  if (value === void 0 || value === null || value === "") return void 0;
  if (!["string", "number"].includes(typeof value) || typeof value === "string" && !value.trim()) {
    throw new PublicInputError("Presupuesto inv\xE1lido.");
  }
  const number2 = Number(value);
  if (!Number.isFinite(number2) || number2 < 0 || number2 > Number.MAX_SAFE_INTEGER) {
    throw new PublicInputError("Presupuesto inv\xE1lido.");
  }
  return number2;
}
...
var json = (body, status = 200) => new Response(JSON.stringify(body), { status, headers: corsHeaders });
var sanitizePublicDescription = (value) => {
  if (typeof value !== "string") return "";
  let text2 = value;
  for (let pass = 0; pass < 3; pass++) {
    text2 = text2.replace(/&#(x[0-9a-f]+|\d+);?/gi, (match, code) => {
      const point = code[0].toLowerCase() === "x" ? parseInt(code.slice(1), 16) : Number(code);
      return point > 0 && point <= 1114111 ? String.fromCodePoint(point) : "";
    }).replace(/&(amp|lt|gt|quot|apos|nbsp);/gi, (_, name) => ({ amp: "&", lt: "<", gt: ">", quot: '"', apos: "'", nbsp: " " })[name.toLowerCase()]);
  }
  text2 = text2.replace(/<script\b[^>]*>[\s\S]*?<\/script\s*>|<style\b[^>]*>[\s\S]*?<\/style\s*>/gi, "").replace(/<br\s*\/?>|<\/(?:p|div|li|h[1-6])\s*>/gi, "\n").replace(/<[^>]*>/g, "").replace(/[\u200b-\u200f\u202a-\u202e\u2060\ufeff]/g, "").replace(/\r\n?/g, "\n");
  const kept = [];
  for (const line of text2.split("\n")) {
    const normalized2 = line.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    ...
    if (privateLabel.test(normalized2) || email2.test(normalized2) || mobile.test(normalized2)) break;
    kept.push(line);
  }
  return kept.join("\n").replace(/\n{3,}/g, "\n\n").trim();
};
var toPublicProperty = (property) => ({
  id: property.id,
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (!["list", "detail"].includes(action) || url.search.length > 4096) {
        throw new PublicInputError("Consulta inv\xE1lida.");
      }
      for (const [key2, max] of [["id", 100], ["custom_id", 100], ["listing_type", 80], ["property_type", 80], ["search", 200]]) {
        publicText(url.searchParams.get(key2), max);
      }
      for (const key2 of ["price_min", "price_max", "area_min", "area_max", "bedrooms"]) {
        optionalBudget(url.searchParams.get(key2));
      }
      const svc = createClientFromRequest(req).asServiceRole;
      if (action === "detail") {
        const id4 = url.searchParams.get("id");
        const customId = url.searchParams.get("custom_id");
        let property;
        if (id4) {
          property = await svc.entities.Property.get(id4);
        } else if (customId) {
