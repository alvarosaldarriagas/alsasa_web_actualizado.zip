...
async function test() {
    const res = await fetch('https://admin.alsasa.co/wp-json/wp/v2/estate_property?per_page=10');
    ...
    data.forEach(item => {
        ...
        let baths = '-';
        const bathsMatch = fullText.match(/(\d+)\s{0,3}baños?\b/) || fullText.match(/baños?[\s:]*(\d+)/);
        if (bathsMatch) {
            baths = bathsMatch[1] || bathsMatch[2];
        } else if (fullText.includes('baño')) {
            baths = '1';
        }
